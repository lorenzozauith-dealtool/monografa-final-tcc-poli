...
\usepackage[brazil,english]{babel} \usepackage{graphicx}
\usepackage{caption} \usepackage{booktabs} \usepackage{listings}
\usepackage{xcolor} \usepackage{tikz} \usetikzlibrary{arrows,
positioning}

% Configurações para o pacote listings (códigos-fonte)
\renewcommand{\lstlistingname}{Listagem}
\renewcommand{\lstlistlistingname}{Lista de Listagens} \lstset{
basicstyle=\footnotesize\ttfamily, breaklines=true,
keywordstyle=\bfseries\color{blue!70},
commentstyle=\itshape\color{green!50!black}, stringstyle=\color{orange},
numbers=left, numberstyle=\tiny, numbersep=5pt, language=C++ % (pode ser
ajustado por listagem individual) }

% Informações de dados para CAPA e FOLHA DE ROSTO \titulo{Sistema de
Detecção de Acidentes e Alerta Automático para Motociclistas}
\autor{Nome do Autor} \local{São Paulo} \data{2025}
\instituicao{Universidade de São Paulo \ Escola Politécnica \
Departamento de Engenharia Elétrica} \orientador{Prof. Dr. Nome do
Orientador} \coorientador{Prof. Dr. Nome do Coorientador (se houver)}
\preambulo{Monografia apresentada à Escola Politécnica da Universidade
de São Paulo como parte dos requisitos para a conclusão do Curso de
Engenharia Elétrica.}

\begin{document} \pretextual \imprimircapa \imprimirfolhaderosto*

% Dedicatória \begin{dedicatoria} \vspace{\fill} \centering {\em Dedico
este trabalho a todos os motociclistas que inspiraram esta pesquisa e
aos meus familiares pelo apoio incondicional durante todo o
desenvolvimento deste projeto.} \vspace \end{dedicatoria}

% Agradecimentos \begin{agradecimentos} Agradeço primeiramente ao meu
orientador, Prof. Dr. Nome do Orientador, pelo apoio, orientação e
incentivo essenciais para a realização deste trabalho. Aos professores
do Departamento de Engenharia Elétrica da Escola Politécnica, pelas
aulas que forneceram a base teórica utilizada neste projeto.

Agradeço também aos colegas de turma e amigos de laboratório, pelas
discussões produtivas e colaboração nas etapas de testes e validações do
sistema. À minha família, por todo suporte emocional e motivação
constantes, fundamentais para a conclusão desta monografia.

Por fim, registro meu agradecimento à Escola Politécnica da USP e à
Universidade de São Paulo, pelas instalações e recursos
disponibilizados, e por promoverem um ambiente acadêmico de excelência
que tornou este trabalho possível. \end{agradecimentos}

% Epígrafe \begin{epigrafe} \vspace{\fill} \begin{flushright}
\textit{``O sucesso não é final, o fracasso não é fatal: é a coragem de
continuar que conta.''}\ (Winston Churchill) \end{flushright} \vspace
\end{epigrafe}

% Resumo em Português \begin{resumo} Acidentes envolvendo motociclistas
representam uma das maiores causas de mortalidade no trânsito
brasileiro. Segundo a Associação Brasileira de Medicina do Tráfego
(ABRAMET), a cada 39 minutos um motociclista perde a vida no trânsito
...
para a sobrevivência. Estudos do \textit{National Highway Traffic Safety
Administration} (NHTSA) indicam que vítimas de acidentes graves têm até
...
embarcados; detecção automática; resposta emergencial. \end{resumo}

% Abstract in English \begin{resumo}[english] Motorcycle accidents are a
...
systems; automatic crash detection; emergency response. \end{resumo}

% Listas (figuras, tabelas, siglas) \listofilustracoes % Lista de
ilustrações (figuras, gráficos, etc.) \listoftables % Lista de tabelas
\lstlistoflistings % Lista de listagens de código (se aplicável)

\begin{siglas} \item[ABRAMET] Associação Brasileira de Medicina do
Tráfego \item[BLE] \textit{Bluetooth Low Energy} \item[CPU]
\textit{Central Processing Unit} (Unidade Central de Processamento)
\item[ESP32-C3] Família de microcontroladores ESP32 (RISC-V 160~MHz com
Bluetooth 5.0) \item[GPS] \textit{Global Positioning System} (Sistema de
Posicionamento Global) \item[IMU] \textit{Inertial Measurement Unit}
(Unidade de Medição Inercial) \item[NHTSA] \textit{National Highway
Traffic Safety Administration} \item[OCR] \textit{Optical Character
Recognition} (Reconhecimento Óptico de Caracteres) \item[RF] Requisito
Funcional \item[RNF] Requisito Não Funcional \item[YOLO] \textit{You
Only Look Once} (Algoritmo de detecção de objetos em tempo real)
\end{siglas}

\begin{simbolos} \item[$g$] Aceleração gravitacional padrão ($\approx
9{,}81~\text{m/s}^2$). \item[$\omega$] Velocidade angular (em radianos
ou graus por segundo). \item[$\Delta$] Variação de uma grandeza
(diferença entre dois instantes ou estados). \end{simbolos}

\sumario

\textual

% Introdução \chapter{Introdução} \label{cap:introducao}
...
destacam estatísticas da ABRAMET e estudos internacionais\cite{who2018}.
...
acidentes, embora com limitações (por exemplo, dependência de o
...
(licenças médicas, indenizações, etc.), e mitigá-los ou minimizar suas
consequências traz benefícios coletivos. Tecnologias de segurança ativa
...
inclusive por órgãos públicos (por exemplo, em frotas de motofrete ou
...
Essa declaração de problema envolve diversos subproblemas técnicos:
\begin{itemize} \item Como distinguir, via sensores embarcados, um
acidente verdadeiro de eventos cotidianos (buracos, frenagens bruscas
normais, inclinações em curvas)? \item Como garantir que o alerta chegue
rapidamente a um destinatário, mesmo em condições adversas (sinal
celular fraco, motociclista inconsciente, etc.)? \item Que informações
adicionais (além do alerta básico) podem ser fornecidas para melhorar a
eficiência do socorro ou registro do incidente (por exemplo, imagens do
local, identificação de placa de veículo envolvido)? \end{itemize}
...
Em suma, a necessidade abrange: \begin{itemize} \item
\textbf{Motociclistas e familiares}: garantia de socorro rápido em caso
de acidente, mesmo que o condutor esteja incapacitado de pedir ajuda.
\item \textbf{Autoridades e serviços médicos}: recebimento de alerta
geolocalizado para otimizar tempo-resposta. \item \textbf{Seguradoras e
pesquisa}: registro de informações do acidente (dinâmica, imagens) para
processamento posterior, auxiliando em indenizações e estudos de
segurança viária. \end{itemize}
...
deve possuir para ser viável e atrativo. Os principais requisitos de
marketing levantados para o sistema proposto foram: \begin{itemize}
...
orientaram a concepção técnica do projeto.

\section{Requisitos de Engenharia} Com base nas necessidades e
requisitos de alto nível identificados, foram definidos os requisitos de
engenharia do sistema de detecção de acidentes e alerta automático para
motociclistas. Esses requisitos foram divididos em funcionais (RF) e
não-funcionais (RNF), conforme resumido nas Tabelas \ref{tab:reqfunc} e
\ref{tab:reqnfunc}. Os requisitos funcionais descrevem as funções e
comportamentos que o sistema deve realizar. Já os requisitos
...
\begin{table}[htb] \centering \caption{Requisitos Funcionais do Sistema}
\label{tab:reqfunc} \begin{tabular}{p{0.15\textwidth} p{0.78\textwidth}}
\toprule \textbf{Código} & \textbf{Descrição do Requisito Funcional}\
\midrule RF-01 & \textbf{Detecção de evento crítico}: O dispositivo
embarcado (ESP32-C3 + sensor inercial) deve monitorar continuamente
aceleração e rotação da motocicleta, identificando padrões típicos de
queda ou colisão brusca.\ RF-02 & \textbf{Acionamento do módulo de
captura}: Ao detectar um evento de acidente (critério de detecção
atendido), o ESP32-C3 deverá gerar um sinal de ativação via GPIO para
“acordar” o microcontrolador Raspberry Pi Zero 2W, que normalmente
permanece em modo de baixo consumo.\ RF-03 & \textbf{Captura de
imagens/vídeo}: O Raspberry Pi, ao ser despertado, deverá realizar o
registro do contexto do acidente. Isso inclui capturar, em modo
\textit{burst}, uma sequência curta de imagens (ou um vídeo de poucos
segundos) através da câmera grande angular, de forma a obter registro
visual do momento do impacto e instantes subsequentes.\ RF-04 &
\textbf{Transmissão ao smartphone}: Os dados capturados (imagens ou
vídeo, e dados do evento como aceleração pico, horário, etc.) deverão
ser enviados do dispositivo embarcado para o smartphone do usuário via
conexão Bluetooth Low Energy (BLE).\ RF-05 & \textbf{Processamento no
aplicativo móvel}: O aplicativo móvel (smartphone) deve receber os dados
do acidente e então executar processamento adicional. Especificamente,
deve: (a) Extrair de imagens recebidas a placa de eventual veículo
envolvido, usando algoritmo de detecção de objetos (p.\,ex. YOLO)
seguido de OCR para ler os caracteres da placa; (b) Compilar uma
mensagem de alerta contendo localização geográfica (via GPS do
smartphone), horário e, opcionalmente, uma foto do acidente com a placa
destacada.\ RF-06 & \textbf{Envio de alerta}: O aplicativo deverá então
enviar automaticamente um alerta de emergência aos destinatários
predefinidos (contatos de confiança do motociclista e/ou serviços de
resgate). Esse alerta pode ser realizado via mensagem SMS, e-mail ou
integração com aplicativos de mensagem (\textit{ex.:} utilizando a API
do Telegram), incluindo as informações relevantes (localização,
identificação do veículo, etc.).\ RF-07 & \textbf{Interação com o
usuário para confirmação}: O sistema deve incluir uma forma do usuário
cancelar o alerta nos casos de falso acionamento ou acidentes sem
gravidade. Por exemplo, o aplicativo móvel pode exibir um aviso sonoro e
visual por alguns segundos antes de disparar a notificação, permitindo
que o motociclista consciente cancele o envio. Caso não haja
cancelamento em tempo hábil, assume-se emergência real e o alerta é
concluído.\ RF-08 & \textbf{Registro de dados contínuo (telemetria)}:
Adicionalmente, o sistema deverá registrar dados de telemetria do
percurso (acelerações extremas, velocidade estimada, eventos de frenagem
brusca) em uma base de dados remota ou local. Esses dados podem ser
usados para análise posterior do comportamento do piloto e para
fornecimento de evidências em caso de acidentes (trajetória antes do
acidente, por exemplo).\ \bottomrule \end{tabular} \end{table}
...
\begin{table}[htb] \centering \caption{Requisitos Não-Funcionais do
Sistema} \label{tab:reqnfunc} \begin{tabular}{p{0.15\textwidth}
p{0.78\textwidth}} \toprule \textbf{Código} & \textbf{Descrição do
Requisito Não-Funcional}\ \midrule RNF-01 & \textbf{Autonomia de
energia}: O sistema embarcado deve operar por pelo menos 10 horas
contínuas sem recarga de bateria, considerando até 2 detecções de
acidente por dia. Esse requisito visa atender o uso típico diário
(trajetos de trabalho, viagens curtas) sem depender de recarga
frequente. \ RNF-02 & \textbf{Robustez física}: A carcaça do dispositivo
deve proteger os componentes contra impactos de colisão e vibrações
intensas. Espera-se que o equipamento continue funcional (ou ao menos
preserve os dados capturados) mesmo após o acidente. Além disso, deve
ter algum grau de proteção ambiental, sendo resistente a poeira e
respingos d'água (equivalente a IP54 ou superior).\ RNF-03 &
\textbf{Desempenho em detecção}: O algoritmo de detecção deve equilibrar
sensibilidade e especificidade, acionando em casos de acidente
verdadeiro (> 95\% de detecções) e evitando falsos alarmes em situações
não críticas (< 5\% de falsos positivos). Esses valores servem como meta
de projeto. Estudos mostram ser comum adotar limiares na faixa de
3--4~$g$ de aceleração para distinguir colisões\cite{choudhary2015},
ajustados experimentalmente para minimizar disparos indevidos. \ RNF-04
& \textbf{Tempo de resposta}: O intervalo de tempo entre a detecção do
acidente e o envio do alerta ao contato de emergência deve ser o menor
possível. A meta estabelecida foi de no máximo 30 segundos para envio da
primeira notificação (mensagem de socorro básica com localização). A
disponibilização de dados complementares (imagens, vídeo) pode ocorrer
em segundo plano, em até 2 minutos, sem comprometer o aviso inicial.
Essa rapidez garante alinhamento com a necessidade de atendimento
imediato.\ RNF-05 & \textbf{Integração e usabilidade do aplicativo}: O
aplicativo móvel deve ser intuitivo e de operação simples. Deve iniciar
automaticamente com o sistema do smartphone e permanecer rodando em
segundo plano enquanto o usuário estiver pilotando, conectando-se ao
dispositivo via BLE sem requerer intervenção manual a cada uso. Em caso
de acidente, a interface deve avisar claramente o usuário e solicitar
confirmação para cancelar ou aceitar o envio do alerta (conforme RF-07).
Também devem ser exibidas informações úteis, como nível de bateria do
dispositivo, status da conexão e último endereço conhecido.\ RNF-06 &
\textbf{Conectividade e alcance}: A comunicação BLE entre o módulo
embarcado e o smartphone deve manter-se estável dentro de um raio de
pelo menos 5~m (considerando que o celular pode estar no bolso do
usuário ou acoplado no guidão). Além disso, o sistema deve tolerar
desconexões temporárias (por exemplo, se o BLE falhar momentaneamente,
tentar reconectar automaticamente). Para o envio de alertas a contatos
remotos, depende-se da conectividade do smartphone (3G/4G/5G ou Wi-Fi);
portanto, o aplicativo deve lidar com indisponibilidade de rede,
enviando assim que houver sinal ou armazenando o alerta para reenvio.\
RNF-07 & \textbf{Compatibilidade e Extensibilidade}: O design do sistema
de software deve facilitar futura compatibilidade multi-plataforma.
Nesta primeira versão o aplicativo foi desenvolvido para iOS (Swift),
mas espera-se que o núcleo da solução seja portátil para Android em
projetos futuros. Os protocolos de comunicação e formatos de dados devem
ser padronizados e documentados (por exemplo, uso de JSON para mensagens
via BLE), garantindo que novos \textit{clients} possam ser implementados
sem necessidade de alterar o dispositivo embarcado.\ \bottomrule
...
\begin{figure}[htb] \centering \begin{tikzpicture}[node distance=1.8cm,
auto] \node[draw, rectangle, align=center] (objetivo-geral)
{\textbf{Objetivo Geral}\Sistema de detecção automática de acidentes de
moto com alerta imediato}; \node[draw, rectangle, below
of=objetivo-geral, yshift=-0.5cm, align=center] (detectar)
{\textbf{Detectar acidentes}\Sensores inerciais)}; \node[draw,
rectangle, below left=1.2cm and -0.6cm of detectar, align=center]
(acelerometro) {Acelerômetro \\(detectar impacto)}; \node[draw,
rectangle, below right=1.2cm and -0.6cm of detectar, align=center]
(giroscopio) {Giroscópio \\(detectar queda)}; \node[draw, rectangle,
below of=detectar, yshift=-0.5cm, align=center] (transmitir)
{\textbf{Transmitir alerta}\\(Comunicação)}; \node[draw, rectangle,
below left=1.2cm and -0.6cm of transmitir, align=center] (bluetooth)
{Bluetooth LE \\(ESP32-C3 \(\rightarrow App)}; \node[draw, rectangle,
below right=1.2cm and -0.6cm of transmitir, align=center] (internet)
{Internet \App \(\rightarrow Contatos)}; \node[draw, rectangle, below
of=transmitir, yshift=-0.5cm, align=center] (registrar)
{\textbf{Registrar evidência}\(Visão computacional)}; \node[draw,
rectangle, below left=1.2cm and -0.6cm of registrar, align=center]
(camera) {Câmera 160° \(imagens/vídeo)}; \node[draw, rectangle, below
right=1.2cm and -0.6cm of registrar, align=center] (ocr) {YOLO + OCR
\(placa veículo)}; \draw[->, thick] (objetivo-geral) -- (detectar);
\draw[->, thick] (detectar) -- (acelerometro); \draw[->, thick]
(detectar) -- (giroscopio); \draw[->, thick] (objetivo-geral) --
(transmitir); \draw[->, thick] (transmitir) -- (bluetooth); \draw[->,
thick] (transmitir) -- (internet); \draw[->, thick] (objetivo-geral) --
(registrar); \draw[->, thick] (registrar) -- (camera); \draw[->, thick]
(registrar) -- (ocr); \end{tikzpicture} \caption{Árvore de objetivos do
projeto, relacionando objetivos específicos aos requisitos
correspondentes.} \label{fig:arvore-objetivos} \end{figure}
...
% Estado da Arte \chapter{Estado da Arte} \label{cap:estado-arte}
...
\section{Conceitos Fundamentais} A detecção de acidentes motociclísticos
de forma automática requer compreender alguns conceitos fundamentais de
engenharia: \begin{itemize} \item \textbf{Sensores inerciais}:
Acelerômetros e giroscópios MEMS são o núcleo de sistemas de detecção de
queda. Um acelerômetro tri-axial mede as componentes de aceleração ao
longo dos eixos X, Y, Z do dispositivo, incluindo tanto aceleração
dinâmica (movimento) quanto a componente estática da gravidade. Um
giroscópio tri-axial mede a velocidade angular (rotação) em torno de
três eixos. A combinação de ambos (em uma IMU) permite inferir a
orientação do veículo e detectar mudanças abruptas de velocidade ou
inclinação. Por exemplo, uma queda lateral de moto envolve tipicamente
rápida variação angular (moto inclinando-se além do ângulo normal de
curva) e desaceleração brusca quando atinge o solo\cite{choudhary2015}.
Esses padrões podem ser identificados processando os sinais do
acelerômetro/giroscópio.

    \item \textbf{Unidades de medida e limiares}: A aceleração é frequentemente expressa em unidades de $g$ (aceleração da gravidade). Impactos severos podem gerar picos de várias vezes $g$. Estudos em detecção de colisões automotivas definem limiares para disparo de airbags na faixa de 3 a 5~$g$ por pelo menos 50~ms, por exemplo. Para motocicletas, um limiar de 4~$g$ foi citado como indicativo de acidente em certo estudo (NHTSA, 2021), ajustado experimentalmente para minimizar disparos indevidos. Em nosso projeto, estabelecemos via testes práticos um valor de aproximadamente 3,5~$g$ como ponto de disparo, levando em conta a sensibilidade do sensor e as dinâmicas típicas de pilotagem. Além do pico de aceleração, utilizamos também um critério de variação angular abrupta (p.\,ex., mudança súbita de inclinação).

    \item \textbf{Comunicação sem fio}: Para transmitir os alertas do dispositivo embarcado até um destinatário remoto, é necessária alguma forma de comunicação sem fio. Duas frentes foram utilizadas: comunicação de curto alcance entre o dispositivo e o smartphone (via Bluetooth Low Energy -- BLE) e comunicação de longo alcance do smartphone com a internet (3G/4G ou Wi-Fi). O BLE foi escolhido por seu baixo consumo de energia e suporte nativo em smartphones modernos, além de perfis apropriados para envio de dados esporádicos (perfil GATT). A velocidade de transferência do BLE é limitada (tipicamente centenas de kb/s), mas suficiente para dados de telemetria e imagens comprimidas. Já o envio de notificações aos contatos remotos aproveita a conectividade do smartphone, seja via redes celulares ou Wi-Fi, por meio de protocolos de internet (HTTP, MQTT, etc.). Assim, o smartphone atua como gateway, encapsulando as informações do acidente e enviando-as a um servidor ou diretamente aos destinatários.

    \item \textbf{Visão computacional embarcada}: Com os avanços em visão computacional, tornou-se possível executar tarefas como detecção de objetos e OCR diretamente em smartphones. \textit{You Only Look Once} (YOLO) é um algoritmo de detecção de objetos em tempo real baseado em redes neurais convolucionais, capaz de localizar e classificar objetos em imagens rapidamente\cite{redmon2018}. Neste projeto, adotamos uma variante moderna dessa família de algoritmos (YOLOv5 durante o desenvolvimento, com possibilidade de migrar para o YOLOv8) para detectar a placa de um veículo envolvido no acidente. Após detectar a placa (região da imagem), aplica-se OCR (reconhecimento óptico de caracteres) para extrair a sequência alfanumérica. Há OCRs especializados em placas, mas frameworks genéricos (como Tesseract ou o \textit{Vision} da Apple) também obtêm bons resultados quando configurados adequadamente. Em suma, combinam-se duas etapas baseadas em IA: detecção da placa via CNN e reconhecimento de texto.

    \item \textbf{Aplicativos móveis e \textit{backend}}: O desenvolvimento de um aplicativo móvel dedicado (no caso, para iOS) permitiu integrar todas as funcionalidades voltadas ao usuário: recebimento de dados via BLE, alerta sonoro/visual de acidente detectado, cancelamento por parte do piloto, e envio de notificações a contatos. Optamos por utilizar um serviço de \textit{backend} em nuvem para intermediar o envio de mensagens (devido a restrições do iOS em enviar SMS/mensagens automaticamente). Foi utilizado o serviço Supabase (BaaS de código aberto) para armazenar os dados dos eventos e acionar um bot do Telegram via API, responsável por enviar a mensagem de alerta ao contato guardião. Esse arranjo contorna limitações de notificações \textit{push} em apps não publicados e provê escalabilidade: múltiplos guardiões podem ser notificados via um canal seguro (Telegram), e os dados ficam registrados na nuvem para consulta posterior.

\end{itemize}

\section{Tecnologias e Soluções Relacionadas} Nesta seção, descrevemos
...
União Europeia (obrigatório em carros novos desde 2018) representa um
marco em sistemas automáticos de chamada de emergência. O \textit{eCall}
...
de trânsito. \textit{WreckWatch}, desenvolvido por White et al. (2011),
foi um dos pioneiros, utilizando o acelerômetro e GPS do telefone para
identificar colisões e enviar alertas geolocalizados\cite{white2011}.
...
poupar bateria). Projetos mais recentes como o \textit{iBump} (2014) e o
\textit{RideSafe} (2018) também implementaram detecção via smartphone,
combinando limiares de aceleração (tipicamente ~3$g$) com detecção de
...
os caracteres. Para nosso caso de uso (acidente), o desafio é realizar
isso em um dispositivo móvel, possivelmente com imagens não ideais
(ângulo, iluminação). Abordagens modernas empregam algoritmos de
detecção como o YOLO para localizar a placa na imagem, seguido de um OCR
para leitura. Khan et al. (2023), por exemplo, treinaram um detector
YOLOv4 específico para placas indianas e alcançaram alta acurácia ao
integrá-lo com um OCR de rede neural para caracteres latinos e
devanágari. Em nosso projeto, utilizamos um modelo YOLO pré-treinado
...
YOLO recentes são eficientes (nossa experiência indicou tempo de
inferência tipicamente < 0,5 s por foto em resolução 1280x720) e a
...
\subsection{Tecnologias Complementares} Algumas outras tecnologias e
tendências merecem menção por se relacionarem ao projeto:
\begin{itemize} \item \textbf{Redes Veiculares (V2X)}: Conexões diretas
entre veículos e infraestrutura (Vehicle-to-X) poderiam, no futuro,
permitir que a própria moto notificasse veículos próximos ou sistemas de
tráfego sobre um acidente. Nosso projeto não aborda V2X, mas é concebido
de forma a poder integrar-se a uma rede dessas, por exemplo enviando o
alerta também a um sistema integrado de trânsito. \item
\textbf{Segurança e Privacidade de Dados}: Embora não seja foco central,
considera-se importante assegurar que os dados coletados (imagens,
localização) sejam usados apenas para a finalidade de emergência. Em
nossa implementação, vídeos sobem para um banco seguro (Supabase) e
mensagens trafegam criptografadas via APIs. Em uma aplicação comercial,
camadas adicionais de segurança seriam necessárias, como criptografia
ponta-a-ponta das evidências enviadas. \item \textbf{Normatização e
Regulamentação}: Para que um sistema desses tivesse adoção ampla,
questões regulatórias deveriam ser consideradas. Na UE, discute-se
estender o eCall para motos (\textit{motorcycle eCall}); no Brasil, não
há exigência legal ainda, mas órgãos como o CONTRAN poderiam vir a
recomendar ou padronizar dispositivos de chamado de emergência em
veículos de duas rodas, dada a magnitude do problema. Este trabalho, ao
demonstrar viabilidade técnica, serve também como subsídio para futuras
normas. \end{itemize}
...
% Materiais e Métodos \chapter{Materiais e Métodos}
...
foi verificar a viabilidade técnica fundamental antes de partir para a
construção do protótipo integrado. Nessa etapa, buscou-se responder às
seguintes perguntas chaves: \begin{itemize} \item Os sensores escolhidos
conseguem detectar de forma confiável um evento de queda/colisão? \item
É possível acionar e operar a câmera e transmitir imagens dentro de um
tempo aceitável após o gatilho do acidente? \item A comunicação entre
dispositivos (ESP32, Raspberry Pi e smartphone) funciona como esperado e
os dados (imagens, texto) são transferidos sem erros? \end{itemize}

Para isso, a PoC foi realizada inicialmente sem a montagem física
definitiva. Foram utilizados os módulos de forma separada, ligados em
bancada: \begin{itemize} \item \textbf{ESP32-C3 DevKit}: Placa de
desenvolvimento do microcontrolador ESP32-C3, conectada a um módulo
sensor \textit{GY-BMI160} (acelerômetro+giroscópio). Foi escrito um
firmware simples que lê continuamente os valores do
acelerômetro/giroscópio via comunicação I2C e imprime na porta serial,
além de aplicar um algoritmo de detecção de pico: se a aceleração
resultante excedesse um limiar inicial (por exemplo, 3$g$) ou se a
inclinação ultrapassasse 60° de forma súbita, o ESP32 registraria um
“evento de acidente”. Nesta fase PoC, ao invés de acionar de fato o
Raspberry Pi, o ESP32 apenas logava a detecção.

    \item \textbf{Raspberry Pi Zero 2 W}: Configurou-se o Raspberry Pi rodando \textit{Raspberry Pi OS Lite} (sem interface gráfica, conhecido como modo \textit{headless}). Inicialmente, testou-se manualmente a capacidade do Pi de capturar imagens da câmera: utilizou-se uma câmera compatível (câmera oficial Raspberry Pi v2 com lente grande angular adaptada) e scripts em Python usando a biblioteca \textit{picamera2} para tirar fotos e salvar em memória. Foi avaliado o tempo de captura (tipicamente < 0,5 s por foto em resolução 1280x720) e a qualidade das imagens. Satisfeitos com isso, integrou-se um código em C++ (compilado no Pi) para interfacear via Bluetooth com o smartphone. Como PoC, instalou-se no RPi um servidor BLE GATT usando a biblioteca BlueZ, expondo um serviço customizado para transferência de arquivo (imagens) sob demanda.

    \item \textbf{Smartphone e App Móvel (protótipo)}: Para a prova de conceito, inicialmente não desenvolvemos o app completo, mas utilizamos um aplicativo de terceiros de testes BLE (como \textit{nRF Connect}) em um iPhone, para assegurar que conseguíamos descobrir o dispositivo Raspberry Pi anunciando no BLE, parear e receber dados. Em paralelo, foram feitos pequenos programas de teste em Swift Playground para verificar o uso do framework Vision (OCR) e CoreML (YOLO). Por exemplo, carregamos um modelo YOLOv5 genérico e testamos detecção em imagens de placas retiradas de um dataset público (Roboflow) para ver se a acurácia era razoável. Também testamos o \textit{VNRecognizeTextRequest} do Vision (framework da Apple para OCR) em imagens de placas brasileiras, confirmando que conseguia extrair corretamente caracteres alfanuméricos nas duas configurações de placa (antiga e Mercosul).

\end{itemize}

Os resultados da PoC foram animadores: o sensor inercial mostrava picos
claros de aceleração e variações de giro nas simulações de acidente, a
câmera do Raspberry Pi capturava imagens rapidamente, e o BLE permitiu
...
pontos (por exemplo, a transferência via BLE de uma imagem \textit{raw}
de 50 kB demorava vários segundos, indicando que compressão ou resolução
...
app iOS disparar mensagens por conta própria (vide problemas com
...
prova de conceito validada, partimos para projetar a arquitetura
...
\begin{figure}[htb] \centering \begin{tikzpicture}[node distance=2.2cm,
auto] \node[draw, rectangle, align=center] (esp)
{\textbf{ESP32-C3}\(Monitora sensor)}; \node[draw, rectangle, below
of=esp, yshift=-0.5cm, align=center] (rpi) {\textbf{Raspberry Pi Zero
2W}\(Captura imagens/vídeo)}; \node[draw, rectangle, below of=rpi,
yshift=-0.5cm, align=center] (cam) {\textbf{Câmera 160°}\(Conectada ao
RPi)}; \node[draw, rectangle, left=3.0cm of esp, align=center] (imu)
{\textbf{Sensor BMI160}\(Acel.+Giro)}; \node[draw, rectangle,
right=3.4cm of rpi, align=center] (phone) {\textbf{Smartphone (App
iOS)}\Recebe dados e envia alerta}; \node[draw, rectangle, below
of=phone, yshift=-0.5cm, align=center] (cloud) {\textbf{Servidor em
Nuvem}\(Base de dados / Storage)}; \node[draw, rectangle, below left=1cm
and -0.3cm of rpi, align=center] (bat) {\textbf{Bateria Li-ion 3.7V}\+
Step-up 5V}; \draw[->, thick] (imu) -- node[anchor=south] {\small
Leitura I2C} (esp); \draw[->, thick] (esp) -- node {\small Sinal GPIO
“Wake”} (rpi); \draw[->, thick] (rpi) -- node[anchor=west] {\small
Controle câmera} (cam); \draw[<->, dashed, thick] (rpi) --
node[align=center, anchor=south] {\small Conexão BLE (dados leves)\
\small \& Wi-Fi (upload mídia)} (phone); \draw[<->, dashed, thick]
(phone) -- node {\small API / Internet} (cloud); \draw[->, dashed]
(cloud) -- ++(0,1.0cm) -| node[pos=0.25,right] {\small Notificação
evento} (phone); \draw[->, thick] (bat) -| node[pos=0.2, above] {\small
5V} (esp); \draw[->, thick] (bat) -| node[pos=0.8, above] {\small 5V}
(rpi); \end{tikzpicture} \caption{Arquitetura simplificada de hardware e
comunicação do sistema proposto. Componentes embarcados (esquerda),
aplicativo móvel (direita) e serviços de nuvem (abaixo) interagem para
detectar acidentes e enviar alertas.} \label{fig:arquitetura}
\end{figure}
...
\section{Componentes de Hardware} \label{sec:hardware} Nesta seção são
descritos os principais módulos de hardware utilizados na implementação
do protótipo, com justificativa das escolhas e parâmetros de
configuração relevantes.

\subsection*{Microcontrolador ESP32-C3 Super Mini} O ESP32-C3 (fabricado
pela Espressif Systems) é um microcontrolador de 32 bits com núcleo
RISC-V rodando a 160~MHz, 400~KB de RAM e conectividade Bluetooth 5.0 LE
...
movimento \textbf{Bosch BMI160}, que combina um acelerômetro triaxial e
...
\subsection*{Microcontrolador Raspberry Pi Zero 2 W} O Raspberry Pi Zero
...
Implementamos monitoramento do nível da bateria via um divisor resistivo
conectado ao ADC do ESP32, permitindo enviar a porcentagem de carga ao
app periodicamente. Também previmos um circuito de proteção para evitar
descarga profunda (a bateria se desconecta ao atingir ~3,0 V).
...
$|\vec{a}| > 3,5g$ (limiar de colisão definido) \textbf{ou} se houve uma
mudança súbita na orientação (detected via integração do giroscópio
indicando $\Delta$ângulo > 60° em curto intervalo), então sinaliza-se um
...
o Supabase Storage (ou outro servidor designado). Em nossos testes, esse
...
para não danificar) em diferentes direções. Em 4 de 4 ensaios, o sistema
...
ocorreu em cerca de 30 eventos testados; o pico de aceleração registrado
nesses casos raramente excedeu 2$g$, abaixo do limiar de detecção. \item
\textbf{Frenagens de emergência}: Simulamos frenagens secas a 40 km/h em
pista fechada. Os picos de desaceleração chegaram a ~3$g$, mas como não
...
simular um impacto a ~15 km/h. O sistema detectou e acionou alerta em 1
caso de 2 (no segundo, a desaceleração foi breve demais para acionar;
...
envio do alerta ao contato guardião ficou em média 25 segundos, dentro
da meta RNF-04. Isso inclui: ~2 s para detecção + ~5 s de
tolerância/confirmar usuário + ~3 s processamento/app + ~5 s upload +
~10 s latência envio Telegram. Em cenários sem cobertura 4G, esse tempo
...
\item Em 20 imagens de teste de placas (10 padrão antigo, 10 Mercosul)
capturadas à noite pelo protótipo, o YOLO identificou corretamente a
região da placa em 18 casos, e o OCR Apple Vision conseguiu ler as 3
letras e 4 números corretamente em 81\% das tentativas sem
pós-processamento. Após aplicarmos nossas correções de formato (troca de
O/0, I/1 e remoção de ruídos), a acurácia subiu para 95\%. Ou seja, o
...
em torno de 0,7 s por frame em um iPhone 12, performance aceitável.
\item Situações adversas: Placas muito sujas ou danificadas não foram
reconhecidas corretamente (tivemos 2 casos de erro do OCR mesmo após
...
reconectou automaticamente em ~10 s após o BLE voltar, conforme
esperado. \item \textbf{Tempo de Upload e Download}: Medimos o tempo de
upload do vídeo nos testes em campo com rede 4G: média de 6,2 s para 3,1
MB. O download do vídeo pelo guardião (via link) levou ~7 s em média.
Esses tempos são adicionais à notificação inicial, mas como discutido, o
alerta básico chega em ~25 s mesmo sem o vídeo. \item \textbf{Segurança
...
app não conseguiu enviar imediatamente; nesses, ele reint tentou após 30
...
% Resultados \chapter{Resultados} \label{cap:resultados}
...
\textbf{Sensibilidade (detecção de acidentes reais)}: Nos 10 cenários
...
colisão muito branda (desaceleração ~2.8$g$) que não atingiu o limiar.
Considerando-se que acidentes graves normalmente envolvem desacelerações
bem acima disso, consideramos que o sistema tem \textbf{90\% de
...
falsos positivos)}: Em mais de 2 horas de pilotagem normal incluindo
buracos, lombadas e frenagens fortes, não houve nenhum alerta falso.
Isso indica \textbf{especificidade de ~100\%} nos cenários testados, ou
...
27,4 segundos, com desvio padrão de 3,1 s entre os testes. Esses valores
cumprem a meta RNF-04 (< 30 s). Em 8 dos 10 casos, o piloto-teste não
cancelou o alerta (simulando estar incapacitado), e a notificação foi
enviada. Nos outros 2, ele cancelou e de fato nenhuma mensagem foi
...
\ \midrule Detecção do acidente (ESP32) & 1,8 \ Confirmação (usuário ou
timeout) & 15,0 \ Processamento e envio no app & 4,2 \ Transmissão na
nuvem (trigger + bot) & 5,5 \ Notificação no Telegram & 0,9 \ \midrule
\textbf{Tempo total (fim detecção até alerta)} & \textbf{27,4} \
\bottomrule \end{tabular} \end{table}

Observa-se que o maior componente é a espera de confirmação do usuário
(15 s, configurado deliberadamente). Esse valor é ajustável: se
...
estacionar e consegue levantá-la de imediato). Optamos por 15 s como
compromisso; ainda assim, 27,4 s atende a RNF-04. Em situações ideais
(usuário inconsciente e ninguém cancela, rede 4G boa), conseguimos
notificar em ~20 s.

Em contrapartida, o tempo de disponibilização do vídeo completo ficou
fora desse intervalo inicial: o guardião recebia o texto do alerta em
~27 s, mas o vídeo podia chegar 10--15 s depois (pois o upload terminava
...
exigiu apenas fixá-lo no guidão e parear o app. Esse processo levou <5
...
3 quedas propositalmente com o protótipo fixado; em nenhuma houve dano
...
9h45min antes que a bateria atingisse 3,3 V. Isso praticamente bate a
...
% Conclusão \chapter{Conclusão} \label{cap:conclusao}
...
propostos. Detectamos 90\% dos acidentes simulados e não registramos
alarmes falsos, cumprindo a meta de sensibilidade/especificidade
(RNF-03). O alerta chegou aos destinatários em menos de 30 segundos,
atendendo RNF-04. O sistema permaneceu operante por cerca de 10 horas
...
reduzir significativamente a mortalidade de motociclistas. Se cada
minuto conta para a sobrevivência, e nosso dispositivo corta
potencialmente dezenas de minutos do tempo de resposta, as chances de
salvar alguém aumentam substancialmente. Além disso, o registro dos
...
\begin{apendices} \chapter{Cronograma Detalhado de Desenvolvimento}
\label{ap:cronograma} O cronograma a seguir detalha as atividades do
projeto ao longo dos dois semestres de desenvolvimento, destacando
marcos importantes e entregas parciais.

\begin{table}[htb] \centering \caption{Cronograma de Desenvolvimento do
Projeto Vindex} \begin{tabular}{p{0.15\textwidth} p{0.75\textwidth}}
\toprule \textbf{Período} & \textbf{Atividades Desenvolvidas (Marcos)}\
\midrule Fev–Mar & Revisão bibliográfica inicial (estado da arte),
definição de escopo e requisitos; Aquisição dos componentes de hardware;
Montagem de setup de bancada para PoC.\ Abr–Mai & Desenvolvimento do
firmware ESP32-C3 (leitura sensor, detecção básica); Testes de PoC com
sensor e LED (simulando alerta); Início do desenvolvimento do app iOS
(conexão BLE básica).\ Jun & Integração inicial ESP32-C3 + app (receber
notificação via BLE); Teste PoC com smartphone e envio de SMS manual;
Apresentação parcial I (Resultados da PoC).\ Jul–Ago & Desenvolvimento
do módulo Raspberry Pi (captura de foto/vídeo); Integração ESP32–RPi
(sinal de wake via GPIO); Desenvolvimento do backend (configuração
Supabase, bot Telegram).\ Set & Implementação completa do aplicativo iOS
(fluxo de alerta com cancelamento, envio ao backend); Montagem do
protótipo físico (PCB, carcaça 3D); Início dos testes integrados
(laboratório).\ Out & Testes de campo (simulações de acidente, ajustes
de sensibilidade); Coleta de resultados quantitativos (tempos,
acurácia); Apresentação parcial II (Protótipo funcional e primeiros
resultados).\ Nov & Análise detalhada dos resultados, comparação com
requisitos; Refinamento de detalhes (ajustes finais de código e
documentação); Escrita da monografia final (capítulos de resultados e
conclusão).\ Dez & Revisão geral do texto, normalização ABNT;
Apresentação final do Trabalho de Conclusão de Curso; Submissão da
monografia e fechamento do projeto.\ \bottomrule \end{tabular}
\end{table}
