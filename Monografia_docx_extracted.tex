...
% Pass additional language options to babel before the class is loaded to avoid option clashes
% Ajuste para compilar no template politeX
\documentclass[]{politeX}
% Codificação de entrada e fonte
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
% Idiomas utilizados (brazil como principal e english como secundário)
...
Lorenzo Bichetti Zauith\\
Rodrigo Costa de Araújo}
\local{São Paulo}
\data{2025}
\instituicao{Universidade de São Paulo \ Escola Politécnica \ Departamento de Engenharia Elétrica}
\orientador{Márcio Lobo Netto}
% Coorientador não se aplica nesta versão
%\coorientador{Nome do Coorientador}
\preambulo{Monografia apresentada à Escola Politécnica da Universidade de São Paulo como parte dos requisitos para a conclusão do Curso de Engenharia Elétrica.}
% Define comando \imprimirtitulo vazio para evitar erro caso não seja fornecido pelo abntex2
...
\begin{dedicatoria}
  ...
  \centering
  {\em Dedico
\emph{Dedicamos este trabalho a todos os motociclistas que inspiraram esta pesquisa e \`as nossas fam\'ilias pelo apoio incondicional durante todo o desenvolvimento deste projeto.}\
\vspace*{\fill}
\end{dedicatoria}
\\begin{agradecimentos}
Agradecemos aos nossos colegas de turma e de laborat\'orio, pelas discuss\~oes proveitosas e pela colabora\c{c}\~ao nas etapas de testes e valida\c{c}\~oes. \`As nossas fam\'ilias, pelo apoio emocional e motiva\c{c}\~ao constantes, fundamentais para a conclus\~ao desta monografia.

Tamb\'em agradecemos \`a Escola Polit\'ecnica da USP e \`a Universidade de S\~ao Paulo pelas instala\c{c}\~oes e recursos disponibilizados, bem como aos professores do Departamento de Engenharia El\'etrica que forneceram a base te\'orica utilizada neste trabalho.

Por fim, registramos nossa gratid\~ao ao Prof.\ Dr.\ M\'arcio Lobo Netto pela orienta\c{c}\~ao e apoio fornecido durante o projeto.

\vspace*{\fill}
\begin{flushright}
\textit{O sucesso n\~ao \'e final, o fracasso n\~ao \'e fatal: \'e a coragem de continuar que conta.}\\
(Winston--Churchill)
\end{flushright}
\end{agradecimentos}
Medicina do Tráfego
...
% Abstract in English
\begin{resumo}[english] Motorcycle accidents are a
...
% Listas (figuras, tabelas, listagens)
\listoffigures   % Lista de ilustrações (figuras, gráficos, etc.)
...
deve possuir para ser viável e atrativo. Os 
\ de
marketing levantados para o sistema proposto 
\iffalse
foram: \begin{itemize}
...
\fi
% Tabela revisada dos requisitos não-funcionais
\begin{table}[htb]
\centering
\caption{Requisitos Não-Funcionais do Sistema}
\label{tab:reqnfunc}
...
moto com alerta imediato};
  ...
  \node[draw, rectangle, below of=objetivo-geral, yshift=-0.5cm, align=center] (detectar)
  {\textbf{Detectar acidentes}\\Sensores inerciais};
  \node[draw, rectangle, below left=1.2cm and -0.6cm of detectar, align=center] (acelerometro)
  {Acelerômetro\\(detectar impacto)};
  \node[draw, rectangle, below right=1.2cm and -0.6cm of detectar, align=center] (giroscopio)
  {Giroscópio\\(detectar queda)};
  \node[draw, rectangle, below of=detectar, yshift=-0.5cm, align=center] (transmitir)
  {\textbf{Transmitir alerta}\\(Comunicação)};
  \node[draw, rectangle, below left=1.2cm and -0.6cm of transmitir, align=center] (bluetooth)
  {Bluetooth LE\\(ESP32-C3 $\rightarrow$ App)};
  \node[draw, rectangle, below right=1.2cm and -0.6cm of transmitir, align=center] (internet)
  {Internet\\(App $\rightarrow$ Contatos)};
  \node[draw, rectangle, below of=transmitir, yshift=-0.5cm, align=center] (registrar)
  {\textbf{Registrar evidência}\\(Visão computacional)};
  \node[draw, rectangle, below left=1.2cm and -0.6cm of registrar, align=center] (camera)
  {Câmera 160°\\(imagens/vídeo)};
  \node[draw, rectangle, below right=1.2cm and -0.6cm of registrar, align=center] (ocr)
  {YOLO + OCR\\(placa veículo)};
...
\end{tikzpicture}
\caption{Árvore de objetivos do
...
. Foram utilizados os módulos de forma separada, ligados em
bancada: 
\item \textbf{ESP32-C3 DevKit}: Placa de
...
irmware simples que lê continuamente os valores do
...
câmera do Raspberry Pi capturava imagens r
\fi
apidamente, e o BLE permitiu
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}[node distance=2.2cm, auto]
    ...
    \node[draw, rectangle, below of=esp, yshift=-0.5cm, align=center] (rpi)
      {\textbf{Raspberry Pi Zero 2W}\\Captura imagens/vídeo};
    \node[draw, rectangle, below of=rpi, yshift=-0.5cm, align=center] (cam)
      {\textbf{Câmera 160°}\\Conectada ao RPi};
    \node[draw, rectangle, left=3.0cm of esp, align=center] (imu)
      {\textbf{Sensor BMI160}\\Acel.+Giro};
    % Blocos de software/aplicativo
    \node[draw, rectangle, right=3.4cm of rpi, align=center] (phone)
      {\textbf{Smartphone (App iOS)}\\Recebe dados e envia alerta};
    \node[draw, rectangle, below of=phone, yshift=-0.5cm, align=center] (cloud)
      {\textbf{Servidor em Nuvem}\\Base de dados / Storage};
    \node[draw, rectangle, below left=1cm and -0.3cm of rpi, align=center] (bat)
      {\textbf{Bateria Li-ion 3.7V}\\+ Step-up 5V};
    % Conexões entre os blocos
    \draw[->, thick] (imu) -- node[anchor=south] {\small Leitura I2C} (esp);
    \draw[->, thick] (esp) -- node {\small Sinal GPIO ``Wake''} (rpi);
    \draw[->, thick] (rpi) -- node[anchor=west] {\small Controle câmera} (cam);
    \draw[<->, dashed, thick] (rpi) -- node[align=center, anchor=south] {\small Conexão BLE (dados leves)\\\& Wi-Fi (upload mídia)} (phone);
    \draw[<->, dashed, thick] (phone) -- node {\small API / Internet} (cloud);
    \draw[->, dashed] (cloud) -- ++(0,1.0cm) -| node[pos=0.25,right] {\small Notificação evento} (phone);
    \draw[->, thick] (bat) -| node[pos=0.2, above] {\small 5V} (esp);
    \draw[->, thick] (bat) -| node[pos=0.8, above] {\small 5V} (rpi);
  \end{tikzpicture}
  \caption{Arquitetura simplificada de hardware e
  ...
  detectar acidentes e enviar alertas.}
...
BMI160\cite{bosch2016} que viabiliza monitoramen\begin{document}
to contínuo, e
...
\begin{figure}[htb]
\centering
% Inserir aqui a imagem do protótipo, se disponível
\caption{Protótipo do sistema desenvolvido}
