...
projeto. A árvore de objetivos apresentada na Figura~\ref{fig:arvore-objetivos} ilustra os objetivos
...
\begin{tikzpicture}[node distance=2.5cm, auto, font=\small]
  ...
  \node[draw, rectangle, below left=2cm and 1.5cm of transmitir, align=center, minimum width=3cm] (bluetooth)
    {Bluetooth LE\\(Node MCU $\rightarrow$ App)};
...
Os resultados da PoC foram animadores: o sensor inercial mostrava picos claros de aceleração e variações de giro nas simulações de acidente, a câmera do Placa Computacional capturava imagens rapidamente, e o BLE permitiu
...
Consequentemente, cogitou-se a troca para o Node MCU-S3 com câmera OV5647 de 5MP, visando implementar um buffer circular de vídeo de aproximadamente 5 segundos. A ideia era captar o acidente antes, durante e após o evento. Contudo, as limitações de memória do Node MCU-S3 (8 MB de PSRAM total) impediram essa abordagem: com qualidade mínima aceitável para OCR (640x480), era possível armazenar apenas 5 segundos de vídeo a 2 frames por segundo, resultando em qualidade insuficiente para reconhecimento de placas. Formatos alternativos como RAW, RGB565 e YUV422 foram testados sem sucesso, uma vez que o Node MCU-S3 não possui poder computacional para executar um encoder de vídeo.

\subsection{Fase V: Arquitetura Final com Upload para Nuvem}

A partir das lições aprendidas, ficou claro que seria necessário retornar ao Placa Computacional como único hardware capaz de gravar vídeo em buffer circular com qualidade suficiente para processamento OCR. O RPi opera com encoder H.264 nativo e tem capacidade de converter para MP4, gerando arquivos compactos adequados para análise rápida.
...
A solução adotada utiliza o Supabase não apenas para alertas, mas também para armazenamento de evidências em vídeo. O fluxo final funciona da seguinte forma: o Node MCU monitora continuamente o acelerômetro e giroscópio; ao detectar um acidente, com um delay de 6 segundos, sinaliza ao RPi para salvar os últimos 8 segundos do buffer circular; o RPi converte o vídeo para MP4 e faz upload para a nuvem; ao concluir o upload, o RPi notifica o Node MCU, que informa ao aplicativo que o vídeo está disponível. O usuário então pode baixar o vídeo da nuvem e o processamento de visão computacional inicia automaticamente.

Esse fluxo completo leva aproximadamente 2 minutos ou menos. O vídeo comprimido é enviado em qualidade 720x30, com tamanho aproximado de 3,5 MB, irrisório para conexões de internet modernas e suficiente para que o OCR reconheça placas de veículos com precisão aceitável.
...
\begin{figure}[p]
  ...
  \begin{tikzpicture}[node distance=2cm, every node/.style={font=\footnotesize}]
    ...
    \node[draw, rectangle, below of=imu, align=center, minimum width=4cm, minimum height=1cm] (esp)
      {\textbf{Node MCU}\\Monitora sensor\\Detecta acidentes};
    
    \node[draw, rectangle, below of=esp, align=center, minimum width=4cm, minimum height=1cm] (rpi)
      {\textbf{Placa Computacional}\\Captura v\'ideo\\Upload nuvem};
    ...
    % Conex\~oes em U do Node MCU para smartphone (BLE)
  ...
  \end{tikzpicture}
  \caption{Arquitetura do sistema Vindex. Hardware embarcado (esquerda): sensor BMI160 conectado ao Node MCU via I2C; Node MCU desperta Placa Computacional via GPIO; RPi controla c\^amera OV5647 via CSI e faz upload de v\'ideo via Wi-Fi. Smartphone (direita): recebe dados via BLE do Node MCU e envia alertas ao servidor Supabase via API REST. Bateria alimenta todos os componentes.}
...
para acordar o Placa Computacional (que permanece em \textit{standby}
para poupar energia). Em seguida, o Node MCU inicia a comunicação
Bluetooth com o smartphone do usuário.

O módulo Placa Computacional (centro) atua quando despertado,
...
circular mantido em memória). A Placa Computacional então comprime o vídeo e
...
No smartphone do usuário (parte direita), um aplicativo iOS
dedicado permanece em segundo plano conectado ao dispositivo via BLE. Ao
receber do Node MCU o sinal de que um acidente foi detectado, o app
...
decisão: \begin{itemize} \item Bluetooth LE embutido: Permite
...
(100 Hz ou mais) e eventualmente repassar dados ao Placa Computacional requer
...
Node MCU aciona imediatamente a linha de \textit{wake} do Placa Computacional
...
giroscópio foi fundamental: implementamos no Node MCU que, se a moto
...
\subsection{Placa Computacional} O Placa Computacional Zero
...
Alternativas consideradas incluíram o Placa Computacional 3A+ (mais potente,
...
Inicialização sob demanda: Configuramos o RPi para iniciar em
...
estado normal, o Node MCU não alimenta o Pi, poupando a bateria; ao
...
versões seria ligado sob demanda), garante que o consumo elevado da
Placa Computacional não interfira na autonomia exceto nos momentos críticos. \item
Sistema minimizado: No Placa Computacional OS removemos serviços
...
resolução continuamente, mas descartando frames antigos. Quando o Node MCU
...
segundos (3 s) para pegar o desfecho. \item Compressão e envio:
Logo após salvar o vídeo (em formato H.264, 720p), o Placa Computacional inicia
...
Nos testes, o Placa Computacional cumpriu seu papel, embora seu consumo seja
...
visuais, utilizamos uma câmera compatível com o Placa Computacional baseada no
...
~9,25 Wh; considerando consumo médio de ~0,9 W (Node MCU ~0,1 W + RPi
...
conectado ao Node MCU, permitindo enviar a porcentagem de carga ao
...
frentes: firmware do Node MCU, scripts e programas no Placa Computacional, e
...
inicia-se um pequeno timer de confirmação: aguarda-se 100 ms
...
Despertar do Placa Computacional: coloca em nível alto o pino
conectado ao gate do MOSFET que alimenta a Placa Computacional, ou envia pulso
conforme circuito configurado. \item Envio BLE para app:
...
Aguardar resposta: o Node MCU espera uma resposta do app
...
Manutenção do estado: após enviar o alerta, o Node MCU permanece
acordado por algum tempo para repassar dados complementares. Por
exemplo, conforme o Placa Computacional fizer upload do vídeo e informar o
link, o Node MCU pode enviar esse link ao app via BLE. \end{enumerate}
...
periodicamente até conseguir conexão. \item Interface para Histórico e Telemetria: O aplicativo também apresenta uma tela com
...
condução. \item Configurações e Perfis: Permite cadastrar o
...
tensão step-up. O Placa Computacional, por sua vez, fica acoplado acima
...
frenagens fortes sem queda. \item Colisão simulada: Amarramos o
...
envio do alerta ao contato guardião ficou em média 28 segundos, dentro
da meta RNF-04. Isso inclui: 0,5 s para detecção + 15,0 s de
tolerância/confirmar usuário + 3,0 s processamento/app + 5,0 s upload +
4,5 s latência envio Telegram. Em cenários sem cobertura 4G, esse tempo
pode aumentar, mas o sistema aguarda até que haja conexão disponível
...
upload do vídeo nos testes em campo com rede 4G: média de 5 s para 3,5
MB. O download do vídeo pelo guardião (via link) levou 8 s em média.
Esses tempos são adicionais à notificação inicial, mas como discutido, o
alerta básico chega em 28 s mesmo sem o vídeo. \item Segurança de Dados: Não implementamos criptografia ponta-a-ponta. Os vídeos sobem
...
mista se justificou. \item Confiabilidade do Alerta (RF-06/RF-07): Em todos os acidentes detectados, o alerta chegou ao
menos a um contato de emergência. O tempo médio de notificação foi de
28 segundos, com desvio padrão de 5 s entre os testes. Esses valores
cumprem a meta RNF-04 (< 30 s). Em 7 dos 10 casos, o piloto-teste não
...
usuário. \item Cobertura de Cenários: A detecção cobriu tanto
...
\midrule
\textbf{Tempo total (fim detecção até alerta)} & \textbf{28,0} \\
\bottomrule
\end{tabular}
\end{table}
...
aumentando brilho via software antes do envio. \item Os vídeos armazenados (720p 8s) revelaram detalhes importantes nas simulações,
...
ligado (com RPi dormindo, Node MCU transmitindo telemetria ocasional) por
...
Contribuições principais: Desenvolvemos um sistema de detecção de acidentes que combina critérios de aceleração superior a 3,5 $g$ e inclinação superior a 65°, permitindo identificar tanto colisões frontais quanto tombamentos laterais. Implementamos arquitetura híbrida com Node MCU para monitoramento contínuo de baixo consumo e Placa Computacional para captura de vídeo sob demanda, otimizando autonomia energética. Integramos visão computacional com YOLO e OCR para identificação automática de placas de veículos envolvidos, funcionalidade ausente em sistemas comerciais de baixo custo. Validamos o sistema em testes controlados, atingindo 90\% de detecção de acidentes reais e 0\% de falsos alarmes, com tempo de resposta de 28 segundos e autonomia de 10 horas.
...
Jul--Ago & Desenvolvimento do módulo Placa Computacional (captura de foto/vídeo); Integração Node MCU–RPi (sinal de wake via GPIO); Desenvolvimento do backend (configuração Supabase, bot Telegram). \\
...
\caption{Tela principal do aplicativo Vindex com localiza\c{c}\~ao, status BLE e score de pilotagem.}
\label{fig:app-tela1}
\end{figure}
...
\caption{Interface de telemetria com gr\'afico de velocidade e estat\'isticas de viagem.}
\label{fig:app-tela2}
\end{figure}
...
\caption{Hist\'orico de viagens com resumo do per\'iodo e lista detalhada.}
\label{fig:app-tela3}
\end{figure}
...
\caption{Tela de evid\^encia com detec\c{c}\~ao de placa (FBR2A23) via YOLO+OCR.}
...
\caption{Tela de evid\^encia com detec\c{c}\~ao de placa (HQW5678) via YOLO+OCR.}
...
\caption{Relat\'orio de viagem com mapa, estat\'isticas e an\'alise de seguran\c{c}a.}
