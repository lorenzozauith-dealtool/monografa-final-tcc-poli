...
\begin{figure}[p]
  \centering
  \begin{tikzpicture}[node distance=1.8cm, every node/.style={font=\footnotesize}]
    % Camada 1: Sensores
    \node[draw, rectangle, align=center, minimum width=3.5cm, minimum height=1.2cm] (imu)
      {\textbf{Sensor BMI160}\\Aceler\^ometro + Girosc\'opio};
    
    % Camada 2: Microcontrolador principal
    \node[draw, rectangle, below of=imu, yshift=-0.5cm, align=center, minimum width=3.5cm, minimum height=1.2cm] (esp)
      {\textbf{ESP32-C3}\\Monitora sensor\\Detecta acidentes};
    
    % Camada 3: Processador de v\'ideo
    \node[draw, rectangle, below of=esp, yshift=-0.5cm, align=center, minimum width=3.5cm, minimum height=1.2cm] (rpi)
      {\textbf{Raspberry Pi Zero 2W}\\Captura v\'ideo\\Upload nuvem};
    
    % Camada 4: C\^amera
    \node[draw, rectangle, below of=rpi, yshift=-0.5cm, align=center, minimum width=3.5cm, minimum height=1.2cm] (cam)
      {\textbf{C\^amera OV5647}\\160° FOV\\720p H.264};
    
    % Camada 5: Bateria
    \node[draw, rectangle, below of=cam, yshift=-0.5cm, align=center, minimum width=3.5cm, minimum height=1.2cm] (bat)
      {\textbf{Bateria Li-ion 18650}\\3.7V + Conversor 5V};
    
    % Lado direito: Smartphone
    \node[draw, rectangle, right=5cm of esp, yshift=-2cm, align=center, minimum width=3.5cm, minimum height=1.8cm] (phone)
      {\textbf{Smartphone iOS}\\App Vindex\\BLE + GPS};
    
    % Lado direito: Nuvem
    \node[draw, rectangle, below of=phone, yshift=-0.5cm, align=center, minimum width=3.5cm, minimum height=1.8cm] (cloud)
      {\textbf{Servidor Supabase}\\Armazenamento\\Bot Telegram};
    
    % Conex\~oes
    \draw[->, very thick] (imu) -- node[right] {I2C} (esp);
    \draw[->, very thick] (esp) -- node[right] {GPIO Wake} (rpi);
    \draw[->, very thick] (rpi) -- node[right] {CSI} (cam);
    \draw[->, very thick] (bat) -- (cam);
    \draw[->, very thick] (bat) -- (rpi);
    \draw[->, very thick] (bat) -- (esp);
    \draw[<->, dashed, very thick] (esp) -- node[above, align=center] {BLE\\Telemetria} (phone);
    \draw[->, dashed, very thick] (rpi) -- node[above, align=center] {Wi-Fi\\Upload v\'ideo} (phone);
    \draw[<->, dashed, very thick] (phone) -- node[right] {API REST} (cloud);
  \end{tikzpicture}
  \caption{Arquitetura do sistema Vindex. Hardware embarcado (esquerda): sensor BMI160 conectado ao ESP32-C3 via I2C; ESP32-C3 desperta Raspberry Pi via GPIO; RPi controla c\^amera OV5647 e faz upload de v\'ideo. Smartphone (direita): recebe dados via BLE e envia alertas. Servidor em nuvem: armazena v\'ideos e aciona bot Telegram.}
