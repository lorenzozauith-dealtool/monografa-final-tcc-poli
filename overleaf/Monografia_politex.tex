...
\begin{figure}[htb]
  \centering
  \begin{tikzpicture}[node distance=2.8cm, auto, font=\small]
    % Blocos de hardware
    \node[draw, rectangle, align=center, minimum width=3.2cm] (esp)
      {\textbf{ESP32-C3}\\Monitora sensor};
    \node[draw, rectangle, below of=esp, yshift=-0.3cm, align=center, minimum width=3.2cm] (rpi)
      {\textbf{Raspberry Pi Zero 2W}\\Captura imagens/v\'ideo};
    \node[draw, rectangle, below of=rpi, yshift=-0.3cm, align=center, minimum width=3cm] (cam)
      {\textbf{C\^amera 160°}\\Conectada ao RPi};
    \node[draw, rectangle, left=3.8cm of esp, align=center, minimum width=2.8cm] (imu)
      {\textbf{Sensor BMI160}\\Acel.+Giro};
    % Blocos de software/aplicativo
    \node[draw, rectangle, right=4.2cm of rpi, align=center, minimum width=3.5cm] (phone)
      {\textbf{Smartphone (App)}\\Recebe dados e\\envia alerta};
    \node[draw, rectangle, below of=phone, yshift=-0.3cm, align=center, minimum width=3.5cm] (cloud)
      {\textbf{Servidor em Nuvem}\\Base de dados /\\Storage};
    \node[draw, rectangle, below left=1.5cm and -0.5cm of rpi, align=center, minimum width=3cm] (bat)
      {\textbf{Bateria Li-ion}\\3.7V + Step-up 5V};
    % Conex\~oes entre os blocos
    \draw[->, thick] (imu) -- node[above] {\footnotesize Leitura I2C} (esp);
    \draw[->, thick] (esp) -- node[right] {\footnotesize Sinal GPIO} (rpi);
    \draw[->, thick] (rpi) -- node[right] {\footnotesize Controle c\^amera} (cam);
    \draw[<->, dashed, thick] (rpi) -- node[above, align=center] {\footnotesize Conex\~ao BLE\\\footnotesize Wi-Fi upload} (phone);
    \draw[<->, dashed, thick] (phone) -- node[right] {\footnotesize API} (cloud);
    \draw[->, dashed] (cloud) -- ++(0,0.8cm) -| node[pos=0.25,right] {\footnotesize Notifica\c{c}\~ao} (phone);
    \draw[->, thick] (bat) -| node[pos=0.15, above] {\footnotesize 5V} (esp);
    \draw[->, thick] (bat) -| node[pos=0.85, above] {\footnotesize 5V} (rpi);
  \end{tikzpicture}
  \caption{Arquitetura simplificada de hardware e comunica\c{c}\~ao do sistema proposto. Componentes embarcados (esquerda), aplicativo m\'ovel (direita) e servi\c{c}os de nuvem (abaixo) interagem para detectar acidentes e enviar alertas.}
