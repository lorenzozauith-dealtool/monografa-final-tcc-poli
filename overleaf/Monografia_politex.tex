...
Alerta instantâneo ao Telegram (< 1s) & 1,0 \\
Confirmação (usuário ou timeout) & 15,0 \\
Delay de gravação (intencional) & 6,0 \\
Processamento de vídeo (Placa Computacional) & 3,0 \\
Upload de vídeo (5G, 4 MB) & 10,0 \\
Download no app (iOS) & 2,0 \\
\midrule
\textbf{Tempo total (detecção até vídeo disponível)} & \textbf{37,5} \\
\bottomrule
\end{tabular}
\end{table}

Observa-se que o alerta de emergência é enviado ao guardião em menos de 1
segundo após a detecção (Fase 3 - Tempo Zero), independentemente do
vídeo. O tempo total de 37,5 segundos refere-se à disponibilização
completa do vídeo de evidência no aplicativo.
