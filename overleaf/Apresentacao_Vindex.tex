...
% Tema e cores - Design profissional
\usetheme{CambridgeUS}
\usecolortheme{dolphin}

% Cores personalizadas Poli-USP
\definecolor{poliblue}{RGB}{0,51,102}
\definecolor{poligold}{RGB}{255,204,0}
\definecolor{darkgray}{RGB}{64,64,64}

\setbeamercolor{structure}{fg=poliblue}
\setbeamercolor{title}{fg=poliblue,bg=white}
\setbeamercolor{frametitle}{fg=white,bg=poliblue}
\setbeamercolor{block title}{fg=white,bg=poliblue}
\setbeamercolor{block body}{fg=black,bg=poliblue!10}
...
% Rodapé customizado - ajustado para não sair da tela
\setbeamertemplate{footline}{
  \leavevmode%
  \hbox{%
  \begin{beamercolorbox}[wd=.25\paperwidth,ht=2.25ex,dp=1ex,left]{author in head/foot}%
    \hspace*{1ex}\usebeamerfont{author in head/foot}\tiny\insertshortauthor
  \end{beamercolorbox}%
  \begin{beamercolorbox}[wd=.5\paperwidth,ht=2.25ex,dp=1ex,center]{title in head/foot}%
    \usebeamerfont{title in head/foot}\tiny\insertshorttitle
  \end{beamercolorbox}%
  \begin{beamercolorbox}[wd=.25\paperwidth,ht=2.25ex,dp=1ex,right]{date in head/foot}%
    \usebeamerfont{date in head/foot}\tiny\insertframenumber{}/\inserttotalframenumber\hspace*{1ex}
  \end{beamercolorbox}}%
  \vskip0pt%
}
...
\title[Vindex]{Sistema de Detecção de Acidentes e Alerta Automático para Motociclistas}
\subtitle{\textbf{Projeto Vindex} \\ Trabalho de Conclusão de Curso}
\author[Lacerda, Zauith, Araújo]{\textbf{Igor Teixeira Lacerda} \\ \textbf{Lorenzo Bighetti Zauith} \\ \textbf{Rodrigo Costa de Araújo}}
\institute[Poli-USP]{Escola Politécnica da Universidade de São Paulo \\ Departamento de Engenharia Elétrica \\ Área: Eletrônica e Sistemas Computacionais}
...
\begin{frame}{Fluxo de Ativação Completo - Timeline}
\small
\textbf{Sequência completa (28 segundos):}

\begin{enumerate}
    \item[\textbf{0,5s}] Node MCU detecta acidente ($|\vec{a}| > 3,5g$ OU inclinação > 65°)
    \item[\textbf{1s}] Node MCU desperta Placa Computacional via GPIO + envia BLE ao app
    \item[\textbf{1,5s}] App exibe alerta: "Toque se você ESTÁ BEM" (15s para cancelar)
    \item[\textbf{7s}] Placa Computacional salva buffer circular (8s de vídeo, 3,5 MB)
    \item[\textbf{16,5s}] Timeout: usuário não cancelou $\rightarrow$ acidente real
    \item[\textbf{17s}] App obtém GPS + processa imagem (YOLO + OCR)
    \item[\textbf{19,5s}] Upload vídeo para Supabase Storage (5s em 4G)
    \item[\textbf{24,5s}] Supabase trigger $\rightarrow$ Bot Telegram formata mensagem
    \item[\textbf{28s}] Guardião recebe alerta no Telegram (localização + vídeo)
\end{enumerate}
\end{frame}
...
\begin{frame}{Contribuições e Inovações}
\begin{block}{Principais Contribuições}
\begin{enumerate}
    \item \textbf{Duplo critério de detecção:} aceleração (3,5g) + inclinação (65°)
    \item \textbf{Arquitetura híbrida:} Node MCU + Placa Computacional (10h autonomia)
    \item \textbf{Visão computacional:} YOLO + OCR (95\% acurácia)
    \item \textbf{Validação:} 90\% detecção, 0\% falsos alarmes, 28s resposta
\end{enumerate}
\end{block}

\vspace{0.3cm}

\begin{block}{Inovações Técnicas}
\begin{itemize}
    \item Evita falsos positivos em frenagens sem queda (100\% especificidade)
    \item Funcionalidade de visão computacional ausente em sistemas de baixo custo
    \item Buffer circular de vídeo (8s) com upload para nuvem
    \item Integração completa: hardware + app + backend + bot Telegram
\end{itemize}
\end{block}
\end{frame}
