...
% Ajustes de formatação ABNT (template politex já fornece a maioria)
% Nenhuma configuração adicional necessária - template politex gerencia ABNT
...
% Siglas e Símbolos (após sumário)
\begin{siglas}
...
\chapter{Interface do Aplicativo iOS}
\label{ap:app}
...
\begin{figure}[htb]
\centering
% Inserir aqui screenshot 1 do aplicativo
\caption{Tela principal do aplicativo Vindex mostrando localiza\c{c}\~ao atual, status da conex\~ao BLE com o dispositivo e n\'ivel de bateria do hardware embarcado}
\label{fig:app-tela1}
\end{figure}

\begin{figure}[htb]
\centering
% Inserir aqui screenshot 2 do aplicativo
\caption{Interface de telemetria exibindo estat\'isticas de viagem, score de qualidade de pilotagem e hist\'orico de eventos registrados pelo sistema}
\label{fig:app-tela2}
\end{figure}

\begin{figure}[htb]
\centering
% Inserir aqui screenshot 3 do aplicativo
\caption{Tela de alerta de acidente com op\c{c}\~ao de cancelamento e visualiza\c{c}\~ao de evid\^encias capturadas (v\'ideo e detec\c{c}\~ao de placa via YOLO+OCR)}
\label{fig:app-tela3}
\end{figure}
